...
import libp2p, libp2p/[stream/connection, crypto/crypto, crypto/secp, multiaddress]
...
proc publishNewMessage(gossipSub: GossipSub, msgSize: int, topic: string): Future[(Time, int)] {.async.} =
  ...
  let useMix = gossipSubMode == Anonymous
  ...
  for chunk in 0..<chunks:
    ...
    res = await gossipSub.publish(topic, nowBytes,
      publishParams = Opt.some(PublishParams(skipMCache: true, useCustomStream: useMix)))
  return (now, res)
...
proc startHttpServer(gossipSub: GossipSub, myId: int): Future[HttpServerRef] {.async.} =
  ...
  proc processRequests(request: RequestFence): Future[HttpResponseRef] {.async.} =
    ...
    try:
      ...
      let responseJson = $(%* {"status": "error", "message": e.msg})
...
proc initializeGossipsub(
    ...
    mixProto: Opt[MixProtocol] = Opt.none(MixProtocol),
): GossipSub =
  let callbacks =
    if mixProto.isSome:
      Opt.some(CustomStreamCallbacks(
        customStreamCreationCB: makeMixConnCb(mixProto.get()),
        customPeerSelectionCB: mixPeerSelection,
      ))
    else:
      Opt.none(CustomStreamCallbacks)
  GossipSub.init(
    switch = switch,
    ...
    anonymize = anonymize,
    rng = rng,
    customStreamCallbacks = callbacks,
  )
...
proc configureGossipsubParams(gossipSub: GossipSub) =
  ...
  gossipSub.parameters.d = GossipSubMeshDegree
  gossipSub.parameters.dLow = 4
  gossipSub.parameters.dHigh = 8
  gossipSub.parameters.dScore = 6
  gossipSub.parameters.dOut = GossipSubMeshDegree div 2
  gossipSub.parameters.dLazy = GossipSubMeshDegree
...
proc main {.async.} =
  ...
  # Generate mix info before SwitchBuilder so withPrivateKey can use the libp2p private key.
  var mixNodeInfo: MixNodeInfo
  if mixEnabled:
  ...
  if mixEnabled:
    builder = builder.withPrivateKey(
      PrivateKey(scheme: Secp256k1, skkey: mixNodeInfo.libp2pPrivKey)
    )
  ...
  if mixEnabled:
    ...
    pingProto = mix_ping.setup(switch, mixProto, rng)
  ...
  if gossipSubMode != Off:
    let mixOpt =
      if mixEnabled and gossipSubMode == Anonymous: Opt.some(mixProto)
      else: Opt.none(MixProtocol)
    gossipSub = initializeGossipsub(switch, true, rng, mixOpt)
  ...
  # Dial peers regardless of gossipSubMode — mix uses the libp2p transport for
  # sphinx hops, so the connections matter even when GossipSub is off.
