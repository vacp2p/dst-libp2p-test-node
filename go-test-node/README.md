...
The [go-libp2p](https://github.com/libp2p/go-libp2p) based test node provides a flexible and extensible environment for GossipSub experiments. It supports yamux/quic transports, and follows the same features and architecture as in the the `nim-libp2p` and `rust-libp2p` [test nodes](https://github.com/vacp2p/dst-libp2p-test-node).
